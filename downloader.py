...
def download_video(url: str, custom_title: str = None, force_filename: str = None) -> str:
    ...
    for strategy in strategies:
        ...
        try:
            ...
            logger.warning("⚠️ [DOWNLOAD] Attempt failed")
            logger.warning(f"    ├─ strategy: {strategy}")
            logger.warning(f"    ├─ error: {str(e)[:100]}")
            
            if strategy == "cookies_file" and "401" in str(e) or "login" in str(e).lower() or "unavailable" in str(e).lower():
                logger.error("❌ YOUR COOKIES.TXT SEEMS INVALID OR EXPIRED. Please refresh it!")
                
            logger.warning(f"    └─ retry: {2}s")
            time.sleep(2)
