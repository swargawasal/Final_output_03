    ...
    def generate_caption(self, image_path: str, style: str = "viral") -> str:
        ...
        try:
            ...
            if not generated_text:
                ...
                # Rotate through fallback list
                # PRIMARY: Use User's Cache (Full History)
                if self.caption_cache and len(self.caption_cache) > 0:
                     generated_text = self.caption_cache[_fallback_index % len(self.caption_cache)]
                     logger.info(f"🔄 Used Cache Fallback #{_fallback_index}: '{generated_text}'")
                else:
