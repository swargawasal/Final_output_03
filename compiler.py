...
def apply_ferrari_composer(
    ...
    f_cinematic = (
        f"eq=brightness=0.02:saturation={1.0 + 0.05*color_intensity}," # Safe pre-gain
        f"curves=master='0/0 0.1/0.08 0.4/0.45 0.7/0.75 0.9/0.92 1/1':" # S-Curve
        f"red='0/0 0.2/0.19 0.5/0.51 0.8/0.81 1/1':" # Subtle warmth
        f"blue='0/0.01 0.2/0.21 0.5/0.49 0.8/0.79 1/0.99'," # Subtle cool shadows
        f"colorbalance="
        f"rs={-0.03*color_intensity}:gs={-0.01*color_intensity}:bs={0.03*color_intensity}:" # Shadows: Teal
        f"rm={0.03*color_intensity}:gm={0.01*color_intensity}:bm={-0.03*color_intensity}:" # Mids: Orange/Gold
        f"rh={0.02*color_intensity}:gh={0.01*color_intensity}:bh={0.01*color_intensity}," # Highs: Neutral/Warm
        f"unsharp=3:3:{sharpen}" # Refined Texture
    )

    # 2. 📸 PAPARAZZI / CLEAN (Flash & Skin Safe)
    # Strategy: Neutral, Bright, High-Fidelity. No color cast.
    f_paparazzi = (
        f"eq=contrast={1.05 + 0.05*color_intensity}:brightness=0.03:saturation={1.02 + 0.02*color_intensity},"
        f"curves=master='0/0 0.3/0.3 0.7/0.72 1/0.98'," # Linear with soft highlight protection
        f"colorbalance=rm={0.01*color_intensity}:bm={-0.01*color_intensity}," # Micro-warmth for skin
    ...
    f_dark_cinema = (
        f"eq=contrast={1.10 + 0.1*color_intensity}:saturation={0.90}," # Desaturated look
        f"curves=master='0/0 0.2/0.15 0.5/0.5 0.85/0.95 1/1'," # Deep shadows
        f"colorbalance="
        f"rs={-0.04*color_intensity}:gs={-0.02*color_intensity}:bs={0.02*color_intensity}:" # Cold Shadows
        f"rm={0.0}:gm={0.0}:bm={-0.02*color_intensity}" # Slide Mids to Gold
    )
    ...
    # Logic: Priority to 'Original_audio' folder (Overrides Fallback Music)
    # This checks for a folder named "Original_audio" in the root directory.
    # If found, it creates a mix from that folder matching the video duration.
    elif os.path.isdir("Original_audio") and len(glob.glob("Original_audio/*.mp3") + glob.glob("Original_audio/*.wav")) > 0:
         logger.info("🎵 'Original_audio' folder detected. Prioritizing over standard music.")
         import audio_processing
         
         # 1. Generate finite mix
         mix_file = os.path.abspath(os.path.join(os.path.dirname(output_path), f"temp_mix_{uuid.uuid4().hex[:6]}.mp3"))
         # Calculate duration needed (Video Duration)
         video_dur = info.get('duration', 0)
         if video_dur == 0: video_dur = 60 # Safety fallback
         
         if audio_processing.create_continuous_music_mix(mix_file, video_dur, music_dir="Original_audio"):
             ...
             bg_source_label = "[s_bg_raw]"
             logger.info(f"✅ Generated & Loaded 'Original_audio' Mix: {mix_file}")
             # Note: mix_file should be deleted after run. We can't easy do it here without a wrapper.
             # Alternatively, we trust temp dir cleanup.
         else:
             # Fallback to standard
             logger.warning("⚠️ Failed to mix 'Original_audio'. Falling back...")
