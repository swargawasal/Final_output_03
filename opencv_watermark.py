...
class FaceProtector:
    ...
    def is_safe_region(cls, frame, box):
        ...
        for (fx, fy, fw, fh) in faces:
            ...
            # Define "Core Zone" (Top 60% - Eyes/Nose protected, allow chin overlap)
            # User Feedback: "Flicker" caused by over-aggressive 85%.
            protected_h = int(fh * 0.60)
    ...
    def clip_masks_for_safety(cls, frame, mask_paths):
        ...
        for (fx, fy, fw, fh) in faces:
             # Scale 0.60 (Relaxed Safety)
             margin_h = int(fh * (1.0 - 0.60) / 2)
             core_h = int(fh * 0.60)
             core_y = fy + margin_h
        ...
        while True:
            ...
            if cv2.countNonZero(final_mask) > 0:
                ...
                if rw > 0 and rh > 0:
                    ...
                    for (fx, fy, fw, fh) in faces:
                         # Define "Core Zone" (Top 60% of face)
                         # Relaxed from 85% to prevent flickering on valid watermarks near chin.
                         core_h = int(fh * 0.60)
                         core_y = fy
