...
def detect_watermark(frames: List[np.ndarray], keywords: str = None) -> Optional[List[Dict[str, int]]]:
    ...
    try:
        ...
        prompt = f"""
SYSTEM ROLE:
You are a forensic watermark detection engine operating in MAXIMUM SENSITIVITY MODE.
...
CRITICAL RULES:
- Assume a watermark EXISTS unless you are 100% certain it does not
- Treat corner regions as HIGH-RISK by default
- Detect even a SINGLE CHARACTER if it appears intentional
- Do NOT ignore faint, partial, or cut-off text
- Do NOT optimize for aesthetics or cleanliness
- Prefer FALSE POSITIVES over FALSE NEGATIVES
- Never respond with “no watermark” unless absolutely certain

INSTRUCTIONS:
Analyze these {len(frames)} frames. Return a JSON list of detected items.
If you are unsure whether something is a watermark, TREAT IT AS A WATERMARK.
